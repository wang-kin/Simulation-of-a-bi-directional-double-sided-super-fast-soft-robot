...
% l10x_ = 0.43E-2*cos(1.7);            % front link to foot x distance (m)
% l10y_ = 0.43E-2*sin(1.7);            % front link to foot y distance (m)
% l10x_ = 0.43E-2*cos(2.6);            % front link to foot x distance (m)
% l10y_ = 0.43E-2*sin(2.6);            % front link to foot y distance (m)
...
for n = length(freqs):-1:1
    ...
    SPEs{n} = 0.5*kspring*(xout(:,3)-xout(:,4)-ang).^2 + 0.5*(ptsy{n}(:,1).^2.*(ptsy{n}(:,1)<0))*ky*ms ...
        + 0.5*(ptsy{n}(:,5).^2.*(ptsy{n}(:,5)<0))*ky*ms; % hinge spring potential energy (J)
