...
% Parameters 
syms l10x l10y l12 l21 l20x l20y l30x l30y% kinematic parameters
...
foot3 = cg1 + [l30x*cos(th1) + l30y*sin(th1); % front foot point
               l30x*sin(th1) - l30y*cos(th1)];
...
m1_ = 40E-6*ms;             % front link mass (kg)
m2_ = 20E-6*ms;             % rear link mass (kg)
l12_ = 0.75E-2;             % front link to pin length (m)
l10x_ = 0.43E-2*cos(1.3);            % front link to foot x distance (m)
l10y_ = 0.43E-2*sin(1.3);            % front link to foot y distance (m)
...
l30x_ = 0.43E-2*cos(-1.3);            % front link to foot x distance (m)
l30y_ = 0.43E-2*sin(-1.3);            % front link to foot y distance (m)
...
I1_ = 1/12*m2_*(2*0.625E-2)^2 + m2_*0.125E-2^2 + 10E-6*ms*0.25E-2^2*2;  
I2_ = 1/12*m2_*(2*0.625E-2)^2;  % [-] rear link moment of inertia (kg m^2)

params1 =  [m1, m2, I1, I2, g, l12, l10x, l10y, l21, l20x, l20y, l30x, l30y];
params1_ = [m1_,m2_,I1_,I2_,g_,l12_,l10x_,l10y_,l21_,l20x_,l20y_, l30x_,l30y_];
...
foot3_ = subs(foot3,params1,params1_);
...
ffoot3 = dynamicsFunction(foot3_,X);
...
for n = length(freqs):-1:1
    ...
    ptisx1x{n} = zeros(length(tint{n}),2);
    ptisy1y{n} = zeros(length(tint{n}),2);
    ...
    for ii = 1:length(tint{n})
        ...
        pt1 = [xint{n}(ii,1:2)', ffoot3(tint{n}(ii),xint{n}(ii,:))]
        ptisx{n}(ii,:) = pt(1,:);
        ptisy{n}(ii,:) = pt(2,:);
        ptisx1x{n}(ii,:) = pt1(1,:);
        ptisy1y{n}(ii,:) = pt1(2,:);
...
KEee=KEs{toPlot}(:,1)+KEs{toPlot}(:,2);
PEee=PEs{toPlot}(:,1)+PEs{toPlot}(:,2);
...
for ii = 1:2500%find(tint{n}>0.2,1,'first'):min(length(tint{n}),find(tint{n}>0.2,1,'first')+300)%1:2500%
    ...
    set(h1,'xdata',xScale*ptisx1x{n}(ii,:),'ydata',xScale*ptisy1y{n}(ii,:))
